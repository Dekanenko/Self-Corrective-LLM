...
class SelfCorrectionDataCollator:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        hallucination_labels = [
            feature.pop("hallucination_labels") for feature in features
        ]
        ...
        batch["hallucination_labels"] = torch.tensor(
            [
                hl + [self.label_pad_token_id] * (max_length - len(hl))
                for hl in hallucination_labels
            ]
        )
...
class SelfCorrectionTrainer(Trainer):
    def __init__(
        self, *args, alpha=0.5, correction_weights: List[float] = None, **kwargs
    ):
        """
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        weight_tensor = (
            self.correction_weight_tensor.to(hallucination_logits.device)
            if self.correction_weight_tensor is not None
            else None
        )
        loss_fct_hallucination = nn.CrossEntropyLoss(
            weight=weight_tensor, ignore_index=-100
        )
        ...
        shift_hallucination_logits = shift_hallucination_logits.view(
            -1, num_correction_classes
        )
        shift_hallucination_labels = shift_hallucination_labels.view(-1).to(
            shift_hallucination_logits.device
        )

        hallucination_loss = loss_fct_hallucination(
            shift_hallucination_logits, shift_hallucination_labels
        )
        ...
        if self.state.is_local_process_zero:
            if model.training:
                ...
                active_preds = torch.argmax(shift_hallucination_logits, dim=-1)[
                    active_labels_mask
                ]
                active_labels = shift_hallucination_labels[active_labels_mask]

                self._eval_accumulator["token_losses"].append(token_loss.item())
                self._eval_accumulator["hallucination_losses"].append(
                    hallucination_loss.item()
                )
    ...
    def evaluate(
        ...
        if self.state.is_local_process_zero:
            ...
            if token_losses:
                output.metrics[f"{metric_key_prefix}_token_loss"] = sum(
                    token_losses
                ) / len(token_losses)
            if hall_losses:
                output.metrics[f"{metric_key_prefix}_hallucination_loss"] = sum(
                    hall_losses
                ) / len(hall_losses)
            if labels:
