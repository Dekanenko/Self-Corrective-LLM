...
class RAPRAgent:
    ...
    def generate_queries(self, state: RAPRState) -> RAPRState:
        ...
        prompt = GenerateQueriesPrompt(input_variables=[
            "question", "statement",
        ])
    ...
    def retrieve_evidence(self, state: RAPRState) -> RAPRState:
        ...
        prompt = RetriveEvidencePrompt(input_variables=[
            "query", "context",
        ])
    ...
    def revise(self, state: RAPRState) -> RAPRState:
        ...
        agreement_prompt = AgreementPrompt(input_variables=[
            "statement", "evidence", "query",
        ])
        agreement_chain = self.eval_llm

        for i, (query, evidence_item) in enumerate(zip(queries, evidence)):
            logger.info(f"Checking agreement for query: {query} and evidence: {evidence_item}")
            ...
            if should_edit:
                edit_prompt = RefineResponsePrompt(input_variables=[
                    "statement", "evidence", "query",
                ])
