...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Resume training from a Stage 1 checkpoint."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the model.tar.gz from Stage 1.",
    )
    ...
    parser.add_argument(
        "--alpha_s2", type=float, default=0.3, help="Alpha for Stage 2."
    )
    ...
    parser.add_argument(
        ...
        help="JSON string for correction weights.",
    )
    parser.add_argument(
        "--gradient_accumulation_steps_s1",
        type=int,
        default=8,
        help="Gradient accumulation for Stage 1 (Unused).",
    )
    parser.add_argument(
        "--gradient_accumulation_steps_s2",
        type=int,
        default=4,
        help="Gradient accumulation for Stage 2.",
    )
    ...
    if local_rank == 0:
        ...
        if args.stage_1_checkpoint_path.startswith("s3://"):
            print(
                f"--- Downloading and extracting S3 tarball from {args.stage_1_checkpoint_path} ---"
            )
            ...
            print(
                f"--- Unpacking local tarball from {args.stage_1_checkpoint_path} ---"
            )
    ...
    if last_checkpoint is None:
        print(
            f"--- Rank {local_rank}: No 'checkpoint-xxx' directory found. Assuming adapter is in {checkpoint_dir} ---"
        )
        last_checkpoint = checkpoint_dir
    else:
        print(
            f"--- Rank {local_rank}: Found latest checkpoint in unpacked directory: {last_checkpoint} ---"
        )
    ...
    model_s2 = PeftModel.from_pretrained(
        model_s2_base, last_checkpoint, is_trainable=True
    )
