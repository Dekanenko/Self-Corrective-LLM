                ...
                formatted_samples.append(
                    {
                        ...
                        "correct_response": sample["corrected_responses"][
...
def push_dataset_to_hub(dataset: List[Dict[str, Any]], repo_name: str):
    ...
    if not token:
        raise ValueError(
            f"Hugging Face token not found. Set the {HUGGINGFACE_TOKEN_ENV_VAR} environment variable."
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Prepare training data and optionally push to Hugging Face Hub."
    )
    parser.add_argument(
        "--config",
        type=str,
        default="configs/push_training_data_config.json",
