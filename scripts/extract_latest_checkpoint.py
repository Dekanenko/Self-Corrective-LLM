...
def extract_latest_checkpoint(input_s3_uri: str, output_s3_uri: str):
    ...
    try:
        ...
        logger.info(
            f"Found {len(potential_checkpoint_paths)} checkpoint(s). Using latest: {latest_checkpoint_path}"
        )
        ...
        for root, dirs, files in os.walk(latest_checkpoint_path):
            for file in files:
                ...
                logger.info(
                    f"Uploading {relative_path} to s3://{output_bucket}/{s3_key}"
                )
        ...
        for root, dirs, files in os.walk(latest_checkpoint_path):
            for file in files:
                relative_path = os.path.relpath(
                    os.path.join(root, file), latest_checkpoint_path
                )
